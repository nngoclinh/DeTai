...
const removeBorrow = async (req, res) => {
  ...
  try {
    ...
    const removeBorrowResults = await pool.query(queries.removeBorrow, [
      borrow_id,
    ]);
...
const updateBorrow = async (req, res) => {
  ...
  try {
    const getBorrowById = await pool.query(queries.getBorrowById, [borrow_id]);
    if (!getBorrowById.rows.length) {
      return res.status(409).send("Borrow ID doesn't exist");
    }
    const readerResults = await pool.query(readerQueries.getReaderById, [
      reader_id,
    ]);
